...
$ /usr/local/bin/etcdctl \
  ...
  --cert-file=/etc/flanneld/ssl/flanneld.pem \
  --key-file=/etc/flanneld/ssl/flanneld-key.pem \
  get ${FLANNEL_ETCD_PREFIX}/subnets/172.30.20.0-24
{"PublicIP":"192.168.5.104","BackendType":"vxlan","BackendData":{"VtepMAC":"d6:51:2e:80:5c:69"}}
```
...
$ /usr/local/bin/etcdctl \
  ...
  /kubernetes/network/subnets/172.30.62.0-24
...
当前五个节点分配的 Pod 网段分别是：172.30.92.0-24、172.30.62.0-24、172.30.20.0-24、172.30.66.0-24、172.30.89.0-24。

在各节点上分配 ping 这五个网段的网关地址，确保能通：
